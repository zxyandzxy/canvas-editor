...
export class CommandAdapt {
    ...
    for (let i = startIndex; i <= endIndex; i++) {
      ...
      if (el) {
        switch (attr) {
          ...
          // 字体加粗
          case 'bold':
            el.bold = <boolean | undefined>value
            this.draw.render({ isSetCursor: false })
            break
          ...
          // 删除线
          case 'strikeout':
            el.strikeout = <boolean | undefined>value
            break
            ...
            changeElementList.forEach(el => {
              ...
              if (payload) {
                ...
                if (isTextLikeElement(el)) {
                  el.size =
                    titleOptions[titleSizeMapping[payload as TitleLevel]]
                  el.bold = true
                }
              } else {
                if (el.titleId) {
  ...
  // 广播事件
  public broadcastRangeStyle(payload: IRangeStyle) {
    // 在这里统一获取当前的 range
    const { startIndex, endIndex } = this.range.getRange()
    if (!startIndex || !endIndex) return
    const ydoc = this.draw.getEditorYDoc()
    if (!ydoc || !ydoc.connect) return
    ydoc.rangeStyleChange({ startIndex, endIndex, ...payload })
  }

  // 协同用户选区
  public setUserRange(data: {
    startIndex: number
    endIndex: number
    id: string
    color?: string
    ...
    /**
     * 多用户的情况下一定是需要绑定唯一id，不然会导致异常
     */
    const { startIndex, endIndex, id, color } = data

    if (startIndex < 0 || endIndex < 0 || endIndex < startIndex) return
    this.setLastRange(startIndex, endIndex, id)
    const isReadonly = this.draw.isReadonly()
    if (isReadonly) return
    if (startIndex === endIndex) return
    // 根据 index 获取 domList 设置颜色
    const elementList = this.draw.getElementList()
    // 这里加1的目的是为了协同用户选区的精准性
    for (let i = startIndex + 1; i <= endIndex; i++) {
      if (elementList[i]) {
        elementList[i].highlight = color || '#F5EEA0'
  ...
  // 用户取消选取
  public setUserUnRange(startIndex: number, endIndex: number, userid?: string) {
    if (userid) {
      // 删除相关选区，
      ...
      if (deRange) {
        ...
        // 删除 lastRange 记录
        this.lastRange.splice(
          this.lastRange.findIndex(i => i.id === userid),
          1
        )
      }
      return
  ...
  // 设置旧选区
  public setLastRange(startIndex: number, endIndex: number, id: string) {
    /**
     * 记录选区状态
     *  1. 每次设置新选区，都要重新赋值旧选区
     *  2. 如果旧选区存在，则需要先取消旧选区
     */
    const current = this.lastRange.find(i => i.id === id)
    if (!current) {
      this.lastRange.push({
        startIndex,
        endIndex,
    ...
    if (ls !== -1 && le !== -1) this.setUserUnRange(ls, le)
  ...
  // keydown
  public keydwnHandle(payload: IKeydown) {
    switch (payload.type) {
      case 'Backspace':
      case 'Delete':
      case 'Enter':
        this.contentChangeHandle(<IEditorData>payload.data)
        break
  ...
  // 获取旧选区
  public getLastRange(id: string) {
    return this.lastRange.find(i => i.id === id)
  }
  ...
  public executePageScale(scale: number) {
    if (!scale) return
